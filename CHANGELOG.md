...
> [!NOTE]
> Prepare v1.2.48: update changelog (custom SSO via WebAuthenticationSession, offline fix, embedded social login legacy flow), archive v1.2.47, and bump podspec to 1.2.48.
> 
> - **Release prep: `v1.2.48`**
>   - **Changelog**: Note custom SSO via `WebAuthenticationSession`, offline mode fix, and embedded social login legacy-flow handling (legacy URL detection, `legacyAuthorizeURL`, auto-switch, backward compatibility) in `CHANGELOG.md`.
>   - **Archive**: Move `v1.2.47` entry to `CHANGELOG.old.md`.
>   - **Versioning**: Bump `FronteggSwift.podspec` from `1.2.47` to `1.2.48`.
> 
> <sup>Written by [Cursor Bugbot](https://cursor.com/dashboard?tab=bugbot) for commit 05e0445bd7a280cbed69c37ead76b8f71daa4f73. This will update automatically on new commits. Configure [here](https://cursor.com/dashboard?tab=bugbot).</sup>
<!-- /CURSOR_SUMMARY -->
