...
final class LocalMockAuthServer {
    ...
    func launchEnvironment(
        ...
        if let misroutedCallbackCode {
            env["FRONTEGG_E2E_MISROUTED_CALLBACK_CODE"] = misroutedCallbackCode
        }
        if let misroutedCallbackState {
            env["FRONTEGG_E2E_MISROUTED_CALLBACK_STATE"] = misroutedCallbackState
        }
    ...
    /// Seeds a deterministic auth code into mock state — used to drive the
    /// SDK's deep-link recovery path from e2e tests without running the
    /// full authorize flow.
    func seedAuthCode(
        code: String,
        email: String,
...
private final class MockAuthState {
    ...
    /// Seeds a known auth code into mock state — used by e2e tests that
    /// need to drive the SDK's deep-link recovery path with a deterministic
    /// `code` value rather than one generated by the full authorize flow.
    func seedAuthCode(code: String, email: String, redirectURI: String, state: String) {
        queue.sync {
            authCodes[code] = AuthCode(email: email, redirectURI: redirectURI, state: state)
        }
    }
